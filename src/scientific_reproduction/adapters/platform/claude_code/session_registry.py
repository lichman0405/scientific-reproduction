"""The durable session registry of the Claude Code adapter (DEV-M10-G03).
...
This registry is deliberately *not* the Agent Teams task store (AC-02):
the task store is a transport detail and never the source of truth for
...
class SessionRegistry:
    ...
    against the task store. It rehydrates from a durable snapshot
    (``to_records``/``from_records``), so a replacement adapter over the
    same workspace answers the same identity (13-EXECUTION-MONITOR.md
    SS4).
    """
