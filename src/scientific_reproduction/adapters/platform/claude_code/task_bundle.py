"""The agent-team task mapping of the Claude Code adapter (DEV-M10-G03).

The goal deliverable "agent-team task mapping": how one spawn maps onto
the task-store/subagent machinery. The adapter translates the durable
session identity (the canonical :class:`WorkerSessionHandle`) plus the
frozen role contract (``RoleContract``, DEV-M10-G01) into the canonical
**task bundle** -- the payload the Agent Teams task store runs and the
prompt the subagent fallback executes:
...
Claude-specific rendering (the task description, the ``claude -p``
prompt) happens inside this package.
...
def build_task_bundle(
    ...
    """The canonical task bundle of one spawned session (AC-01 mapping).

    The payload the Agent Teams task store runs and the subagent
    ...
    if not isinstance(handle, WorkerSessionHandle):
        raise TypeError(
            f"build_task_bundle expects a WorkerSessionHandle, got"
            f" {type(handle).__name__}"
        )
    if not isinstance(contract, RoleContract):
        raise TypeError(
            f"build_task_bundle expects a RoleContract, got"
...
def render_subagent_prompt(bundle: dict[str, Any]) -> str:
    """Render the canonical task bundle into the subagent prompt text.

    Deterministic, canonical, newline-joined: the durable identity plus
    the frozen contract directives, so a replacement subagent takes over
    with the same identity and the same contract without chat-memory
    access (13-EXECUTION-MONITOR.md SS4).
    ...
    for key in _BUNDLE_FIELDS:
        ...
        if isinstance(value, list):
            if not all(isinstance(item, str) and item.strip() for item in value):
                raise PlatformAdapterDataError(
                    f"corrupt task bundle: {key!r} entries must be non-empty"
                    " strings"
                )
            ...
            if not value.strip() and key not in ("goal_id", "context_id"):
                raise PlatformAdapterDataError(
                    f"corrupt task bundle: {key!r} must be a non-empty string"
                )
            lines.append(value)
        else:
            raise PlatformAdapterDataError(
                f"corrupt task bundle: {key!r} must be a str or list of str,"
