...
the frozen Goal/Acceptance/StatisticalDesign/Analysis/Closure contracts
and **persists the goal-contract family in place** at the same registry
paths -- after the freeze, any state reader (``read_goal`` /
``read_acceptance`` / ``read_analysis_protocol`` /
``read_closure_contract``) sees the frozen record, never a stale draft;
the revision flow re-opens the frozen family as drafts of the next
version. Registration stays exactly-once: the freeze and revision
transitions are the only writers that may replace a registered record
